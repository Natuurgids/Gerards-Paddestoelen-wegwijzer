...
void main() {
  ...
  test('regional season lookup does not borrow another region calendar', () async {
    final nl = await repository.identify(
      'en',
      {},
      observationMonth: 10,
      seasonRegionCode: 'NL',
    );
    final gb = await repository.identify(
      'en',
      {},
      observationMonth: 10,
      seasonRegionCode: 'GB-IE',
    );
...
Future<void> _seed(Database db) async {
  ...
  await db.insert('species_text', {
    'species_id': 1,
    'language_code': 'en',
    'common_name': 'Alpha mushroom',
    'summary': 'Alpha',
  });
  await db.insert('species_text', {
    'species_id': 2,
    'language_code': 'en',
    'common_name': 'Beta mushroom',
    'summary': 'Beta',
  });
