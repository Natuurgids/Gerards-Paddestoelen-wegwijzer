...
class FieldDataRepository {
  ...
  Future<List<SeasonRegionOption>> seasonRegions(String languageCode) async {
    ...
    return regions.map((rawRegion) {
      ...
      return SeasonRegionOption(code: region['code'] as String, label: localized(labels), note: localized(notes));
    }).toList();
  }
}

class SpeciesRepository {
  SpeciesRepository({DatabaseProvider? databaseProvider}) : _databaseProvider = databaseProvider ?? _defaultDatabaseProvider;
  final DatabaseProvider _databaseProvider;

  Future<List<SpeciesSummary>> search(String languageCode, {String query = ''}) async {
    final db = await _databaseProvider();
    final like = '%${query.trim()}%';
    final rows = await db.rawQuery('''SELECT s.id, t.scientific_name, st.common_name, st.summary,
      ...
      WHERE ?='' OR st.common_name LIKE ? COLLATE NOCASE OR t.scientific_name LIKE ? COLLATE NOCASE ORDER BY st.common_name COLLATE NOCASE''', [languageCode, query.trim(), like, like]);
    return rows.map(_summary).toList();
  }

  Future<SpeciesDetail?> detail(int id, String languageCode) async {
    final db = await _databaseProvider();
    final rows = await db.rawQuery('''SELECT s.id, s.edible_status, s.toxicity_level, t.scientific_name,
      st.common_name, st.summary, st.description, st.habitat_text, st.lookalikes_text,
      (SELECT asset_path FROM species_image si WHERE si.species_id=s.id ORDER BY si.is_primary DESC, si.sort_order LIMIT 1) image_path
      FROM species s JOIN taxon t ON t.id=s.taxon_id JOIN species_text st ON st.species_id=s.id AND st.language_code=? WHERE s.id=? LIMIT 1''', [languageCode, id]);
    ...
    return SpeciesDetail(id: row['id'] as int, scientificName: row['scientific_name'] as String, commonName: row['common_name'] as String,
      summary: row['summary'] as String?, imagePath: row['image_path'] as String?, description: row['description'] as String?, habitat: row['habitat_text'] as String?,
      lookalikes: row['lookalikes_text'] as String?, edibleStatus: row['edible_status'] as String, toxicityLevel: row['toxicity_level'] as String,
  ...
  SpeciesSummary _summary(Map<String, Object?> row) => SpeciesSummary(id: row['id'] as int, scientificName: row['scientific_name'] as String,
    commonName: row['common_name'] as String, summary: row['summary'] as String?, imagePath: row['image_path'] as String?);
}

class IdentificationRepository {
  IdentificationRepository({DatabaseProvider? databaseProvider}) : _databaseProvider = databaseProvider ?? _defaultDatabaseProvider;
  final DatabaseProvider _databaseProvider;

  Future<List<TraitChoice>> choices(String languageCode) async {
    final db = await _databaseProvider();
    final rows = await db.rawQuery('''SELECT tr.id trait_id, tr.code trait_code, COALESCE(tt.label, tr.category) trait_label,
      ...
      WHERE tr.value_type='choice' ORDER BY tr.id, o.sort_order''', [languageCode, languageCode]);
    return rows.map((r) => TraitChoice(traitId:r['trait_id'] as int, traitCode:r['trait_code'] as String, traitLabel:r['trait_label'] as String,
      optionId:r['option_id'] as int, optionLabel:r['option_label'] as String)).toList();
  }

  Future<List<IdentificationCandidate>> identify(String languageCode, Map<int,int> selected, {int? observationMonth, String? seasonRegionCode,
    double? capDiameterCm, double? stemHeightCm, double? stemDiameterCm}) async {
    final morphology = await _morphologyCandidates(languageCode, selected);
    final fieldRequested = (observationMonth != null && seasonRegionCode != null ? 1 : 0) + (capDiameterCm != null ? 1 : 0) +
        (stemHeightCm != null ? 1 : 0) + (stemDiameterCm != null ? 1 : 0);
  ...
  Future<List<IdentificationCandidate>> _morphologyCandidates(String languageCode, Map<int,int> selected) async {
    ...
    final db = await _databaseProvider(); final valueSql = List.filled(selected.length, '(?, ?)').join(', '); final args = <Object?>[];
    for (final entry in selected.entries) { args..add(entry.key)..add(entry.value); } args.add(languageCode);
    final rows = await db.rawQuery('''WITH selected(trait_id, option_id) AS (VALUES $valueSql),
      per_trait AS (
      ...
      WHERE scores.matched_weight > 0 ORDER BY score DESC, scores.matched_count DESC, txt.common_name COLLATE NOCASE LIMIT 50''', args);
    return rows.map((r) => IdentificationCandidate(species: SpeciesSummary(id:r['id'] as int, scientificName:r['scientific_name'] as String,
      commonName:r['common_name'] as String, summary:r['summary'] as String?, imagePath:r['image_path'] as String?), score:(r['score'] as num).toDouble(),
      matched:(r['matched_count'] as num).toInt(), requested:selected.length)).toList();
  }
}

class TrainingRepository {
  TrainingRepository({DatabaseProvider? databaseProvider}) : _databaseProvider = databaseProvider ?? _defaultDatabaseProvider;
  final DatabaseProvider _databaseProvider;
  Future<List<LessonSummary>> lessons(String languageCode) async { final db = await _databaseProvider();
  ...
  Future<List<QuizQuestion>> questions(int lessonId, String languageCode) async { final db = await _databaseProvider();
    ...
    final result=<QuizQuestion>[]; for(final q in qs){ final answers=await db.rawQuery('''SELECT a.id,a.is_correct,t.label FROM answer_option a JOIN answer_option_text t ON t.answer_id=a.id AND t.language_code=? WHERE a.question_id=? ORDER BY a.sort_order''',[languageCode,q['id']]);
      result.add(QuizQuestion(id:q['id'] as int,prompt:q['prompt'] as String,explanation:q['explanation'] as String?,answers:answers.map((a)=>QuizAnswer(id:a['id'] as int,label:a['label'] as String,isCorrect:(a['is_correct'] as int)==1)).toList())); } return result; }
  Future<void> saveScore(int lessonId,double score) async { final db=await _databaseProvider(); final current=await db.query('training_progress',where:'lesson_id=?',whereArgs:[lessonId],limit:1);
    final oldBest=current.isEmpty?0.0:(current.first['best_score'] as num).toDouble(); final attempts=current.isEmpty?0:current.first['attempts'] as int;
