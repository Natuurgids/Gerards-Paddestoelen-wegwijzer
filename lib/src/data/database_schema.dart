...
class DatabaseSchema {
  ...
  static Future<void> upgrade(
    ...
    if (oldVersion < 3) {
      ...
      await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_species_measurement_lookup ON species_measurement(measurement_code, min_value, max_value, species_id)',
      );
      await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_species_season_month ON species_season(month, likelihood, species_id)',
      );
    }
    ...
    if (oldVersion < 5) {
      ...
      await db.execute(
        '''INSERT OR IGNORE INTO species_season_v5(species_id, region_code, month, likelihood)
